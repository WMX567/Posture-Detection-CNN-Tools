...
def calculate_vector_sensor_right_17(x, y, z, bool_values):
    ...
    if bool_values[1]:
        vec_head = [x[9]-x[8], y[9]-y[8], z[9]-z[8]]
    if bool_values[2]:
        vec_arm = [x[11]-x[10], y[11]-y[10], z[11]-z[10]]
    if bool_values[3]:
        vec_elbow = [x[12]-x[11], y[12]-y[11], z[12]-z[11]]
    if bool_values[4]:
        vec_leg = [x[15]-x[14], y[15]-y[14], z[15]-z[14]]
    if bool_values[5]:
        vec_knee = [x[16]-x[15], y[16]-y[15], z[16]-z[15]]
    return vec_back, vec_head, vec_arm, vec_elbow, vec_leg, vec_knee
